...
- **Global scope**: `/home/theanh/meetmind/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/theanh/meetmind/.claude/notes/`)
...
2. Glob `/home/theanh/meetmind/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/theanh/meetmind/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/theanh/meetmind/.claude/notes/` — works in any directory
