          ...
          chatMessages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id || idx} className={`flex gap-3 ${isSelf ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`h-9 w-9 rounded-xl flex items-center justify-center text-[10px] font-bold shrink-0 overflow-hidden ${isSelf ? 'bg-cyan-600 text-white shadow-[0_0_15px_rgba(8,145,178,0.3)]' : 'bg-white/10 text-slate-200 border border-white/10'}`}>
                   {avatarUrl ? (
                ...
                <div className={`max-w-[85%] flex flex-col ${isSelf ? 'items-end' : 'items-start'}`}>
                   <div className="flex items-center gap-3 mb-2 px-1">
                      <span className="text-sm font-bold text-white">{msg.from?.name || msg.from?.identity}</span>
                      <span className="text-[10px] font-medium text-slate-500">{formatTime(msg.timestamp)}</span>
                   </div>
                  <div className={`px-4 py-3 rounded-[1.25rem] text-[14px] leading-relaxed shadow-sm ${isSelf ? 'bg-cyan-600 text-white' : 'bg-white/5 border border-white/5 text-slate-200'}`}>
                    {msg.message}
                  </div>
                </div>
              </div>
