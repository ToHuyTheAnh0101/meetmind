...
const MeetingSidebar: React.FC<MeetingSidebarProps> = ({
  ...
  return (
    <div
      className={`fixed right-0 top-0 bottom-0 h-full flex z-50 lg:relative lg:right-auto lg:top-auto lg:bottom-auto lg:z-40 overflow-hidden transition-all duration-300 ${!isOpen ? "w-0 lg:w-[64px]" : "w-full lg:w-auto"}`}
    >
                  ...
                  {breakoutRooms.length > 0 ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold text-teal-400/80 tracking-wider">
                          {t("meeting.active_rooms", "Phòng đang hoạt động")}
                        </span>
                        <button
                          ...
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-500/20 text-teal-400 border border-teal-500/20">
                                {room.status === "active"
                                  ? t(
                                      "meeting.status.ongoing_short",
                                      "Đang họp",
                                    )
                                  : t(
                                      "meeting.status.completed_short",
                                      "Đã kết thúc",
                                    )}
                              </span>
                            </div>
                                  ...
                                  >
                                    <div className="w-4 h-4 rounded-md bg-teal-500/20 flex items-center justify-center text-[8px] text-teal-400 overflow-hidden relative">
                                      <img
                                        src={
                                          p.user?.picture ||
                                          `https://ui-avatars.com/api/?name=${encodeURIComponent(p.user?.firstName || "U")}&background=random&color=fff`
                                        }
                                        alt=""
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                      ...
                      <h4 className="text-lg font-bold text-white mb-2">
                        {t(
                          "meeting.breakout_rooms_title",
                          "Chia phòng họp nhỏ",
                        )}
                      </h4>
                      <p className="text-sm text-slate-400 mb-8 leading-relaxed">
                        {t(
                          "meeting.breakout_rooms_desc",
