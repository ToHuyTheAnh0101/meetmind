...
import { BreakoutRoom, BreakoutRoomStatus } from '../entities/collaboration/breakout-room.entity';
...
export class BreakoutRoomService {
  ...
  async setupBreakoutRooms(
    ...
    for (const roomDto of dto.rooms) {
      ...
      console.log(`Created room ${savedRoom.name} with ${savedRoom.participants?.length || 0} participants`);
    }

    // 3. Lấy lại toàn bộ danh sách phòng kèm participants để trả về
    const finalRooms = await this.breakoutRoomRepository.findByMeetingId(meetingId);
  ...
  async startBreakout(meetingId: string, userId: string) {
    ...
    if (meeting.organizerId !== userId) throw new ForbiddenException('Only organizer can start breakout');
  ...
  async endBreakout(meetingId: string, userId: string) {
    const meeting = await this.meetingRepository.findById(meetingId);
    if (!meeting) throw new NotFoundException('Meeting not found');
    if (meeting.organizerId !== userId) throw new ForbiddenException('Only organizer can end breakout');

    await this.breakoutRoomRepository.removeAllForMeeting(meetingId);
    console.log(`[DEBUG] All breakout rooms for meeting ${meetingId} have been hard deleted.`);

    return { message: 'Breakout rooms closed' };
  }

  async getBreakoutToken(meetingId: string, userId: string) {
    console.log(`[DEBUG] getBreakoutToken: searching for userId="${userId}" in meetingId="${meetingId}"`);
    const rooms = await this.breakoutRoomRepository.findByMeetingId(meetingId);
    ...
    let foundParticipant: any = null;

    for (const r of rooms) {
      console.log(`[DEBUG] Checking Room: "${r.name}" | Status: ${r.status} | Participants Count: ${r.participants?.length}`);
      const p = r.participants?.find(part => 
        String(part.userId).toLowerCase().trim() === String(userId).toLowerCase().trim()
      );
      
      if (p) {
        console.log(`[DEBUG] Found user in room "${r.name}". Room status is ${r.status}`);
        foundParticipant = p;
    ...
    if (!foundRoom || !foundParticipant) {
      console.log(`[DEBUG] No ACTIVE room found for user ${userId}. (RoomFound=${!!foundRoom}, ParticipantFound=${!!foundParticipant})`);
      return null;
    }
    ...
    // Tên hiển thị
    const participantName = `${participant.user?.firstName || 'User'} ${participant.user?.lastName || ''}`.trim();
